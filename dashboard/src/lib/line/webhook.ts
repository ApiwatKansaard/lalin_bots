...
import { extractSlipData, verifySlip } from './slip-verification';
import {
...
async function handleTextMessage(_event: MessageEvent, userId: string): Promise<void> {
  try {
    const text = (_event.message as TextEventMessage).text.trim();
